...
export default async function webhookRoutes(fastify: FastifyInstance) {
  ...
  async function processSms(text: string, from: string | undefined, reply: any) {
    fastify.log.info({ from, preview: text.slice(0, 60) }, 'SMS received');

    // Store in Redis for telegram-bot listener to pick up and forward
    await redisClient.rPush(
      'sms:incoming',
      JSON.stringify({ from: from || 'unknown', text, received_at: new Date().toISOString() })
    );
    ...
    // Store OTP in Redis for bot matching
    await redisClient.rPush(
  ...
  typedFastify.post(
    ...
    async (request, reply) => {
      return processSms(request.body.text, request.body.from, reply);
    }
  );
