...
def upsert_staff(payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    if not required.issubset(payload):
        raise HTTPException(
            status_code=400, detail=f"missing fields: {required - set(payload)}"
        )
...
def set_staff_state(
    staff_id: str, payload: Dict[str, Any] = Body(...)
) -> Dict[str, Any]:
    state = payload.get("state")
    if state not in ("online", "idle", "offline", "disabled"):
        raise HTTPException(status_code=400, detail="invalid state")
    _exec(
        "UPDATE staff SET state = %s, updated_at = now() WHERE id = %s",
        (state, staff_id),
    )
...
def list_capabilities() -> List[Dict[str, Any]]:
    return _query(
        "SELECT code, label, category, risk FROM capability ORDER BY category, code"
    )
...
def capability_matrix() -> Dict[str, Any]:
    staff = _query(
        "SELECT id, kind, display_name, short_name FROM staff ORDER BY kind, display_name"
    )
    caps = _query(
        "SELECT code, label, category, risk FROM capability ORDER BY category, code"
    )
...
def grant_capability(payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    if not staff_id or not code:
        raise HTTPException(
            status_code=400, detail="staff_id and capability_code required"
        )
    _exec(
        """
...
def assign_task(payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    staff = _query(
        "SELECT kind, short_name, agent_module FROM staff WHERE id = %s", (staff_id,)
    )
    if staff and staff[0]["kind"] == "agent":
        logger.info(
            f"Triggering agent {staff[0]['short_name']} for task: {description}"
        )
    ...
    return {
        "ok": True,
        "message": f"Task assigned to {staff[0]['display_name'] if staff else 'unknown'}",
    }


@router.post("/capability/{code}/limit", dependencies=[Depends(_verify_admin)])
def set_capability_limit(
    code: str, payload: Dict[str, Any] = Body(...)
) -> Dict[str, Any]:
    limit = payload.get("per_hour_limit")
    if limit is not None and (not isinstance(limit, int) or limit < 0):
        raise HTTPException(
            status_code=400,
            detail="per_hour_limit must be a non-negative integer or null",
        )
