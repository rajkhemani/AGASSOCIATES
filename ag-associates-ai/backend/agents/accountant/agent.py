"""Accountant Agent — financial reports and billing."""
...
from .prompts import ACCOUNTANT_PERSONA
...
class AccountantAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            name="accountant",
            persona_prompt=ACCOUNTANT_PERSONA,
            required_permission="agent.accountant.access",
        )
    ...
    async def process_request(self, user_message, user_id, user_role="CLERK", conversation_id=None):
        if not self.check_access(user_role):
            return AgentResponse("❌ Accountant agent ke liye aapke paas permission nahi hai.")
