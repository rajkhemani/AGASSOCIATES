"""Executor Agent — RPA automation runner."""
...
from .prompts import EXECUTOR_PERSONA
...
class ExecutorAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            name="executor",
            persona_prompt=EXECUTOR_PERSONA,
            required_permission="agent.executor.access",
        )
    ...
    async def process_request(self, user_message, user_id, user_role="CLERK", conversation_id=None):
        if not self.check_access(user_role):
            return AgentResponse("❌ Executor agent ke liye aapke paas permission nahi hai.")
