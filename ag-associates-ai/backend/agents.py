...
def bouncer_node(state: AgentState) -> AgentState:
    ...
    if duration_str:
        ...
        duration_tokens = re.findall(
            r"(\d+(?:\.\d+)?)\s*([a-zA-Z]*)", duration_str.lower()
        )
        if duration_tokens:
            total_months = 0.0
...
""",
        ...
        result = invoke_llm_with_retry(
            chain,
            {"extracted_data": extracted_str, "template_content": template_content},
        )
...
def build_agent_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        "guardrail", check_guardrail, {"finish": END, "continue": "bouncer"}
    )
    ...
    workflow.add_conditional_edges(
        "bouncer", check_bouncer, {"finish": END, "continue": "drafter"}
    )
    ...
    workflow.add_conditional_edges(
        "auditor", should_revise, {"finish": END, "revise": "drafter"}
    )
...
def process_rental_request(
    raw_input: str, sender: str = "whatsapp_user", org_id: Optional[str] = None
) -> Dict[str, Any]:
    """
