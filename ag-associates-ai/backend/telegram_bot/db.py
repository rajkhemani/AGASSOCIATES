...
def list_cases(status: str = None) -> list:
    with get_conn() as conn:
        with conn.cursor() as cur:
            if status:
                cur.execute(
                    "SELECT id, client_name, status, created_at FROM noi_cases WHERE status = %s ORDER BY created_at DESC LIMIT 20",
                    (status,),
                )
            else:
                cur.execute(
                    "SELECT id, client_name, status, created_at FROM noi_cases ORDER BY created_at DESC LIMIT 20"
                )
            return [
                {
                    "id": str(r[0]),
...
def get_case(case_id: str) -> dict | None:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, client_name, client_contact, description, property_details, status, telegram_group_id, created_at, updated_at FROM noi_cases WHERE id = %s",
                (case_id,),
            )
            r = cur.fetchone()
            if not r:
            ...
            return {
                "id": str(r[0]),
                "client_name": r[1],
                "client_contact": r[2] or "",
                "description": r[3] or "",
                "property_details": r[4] or {},
                "status": r[5],
                "telegram_group_id": r[6],
...
def update_case_status(case_id: str, status: str) -> bool:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE noi_cases SET status = %s WHERE id = %s", (status, case_id)
            )
...
def update_task_status(task_id: str, status: str) -> bool:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE noi_tasks SET status = %s WHERE id = %s", (status, task_id)
            )
            return cur.rowcount > 0
...
def list_challans(case_id: str) -> list:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, amount, description, status, created_at FROM challans WHERE case_id = %s ORDER BY created_at",
                (case_id,),
            )
            return [
                {
...
def approve_challan(challan_id: str, approved_by: str = "") -> bool:
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE challans SET status = 'approved', approved_by = %s WHERE id = %s",
                (approved_by, challan_id),
            )
            return cur.rowcount > 0
