...
class VyasaAgent:
    ...
    def generate_legal_opinion(
        self, case_details: Dict[str, Any], query: str
    ) -> Dict[str, Any]:
        """Generate a legal opinion based on case facts and query."""
        logger.info(
            f"Vyasa researching query for case {case_details.get('case_number', 'UNKNOWN')}"
        )
...
Do not invent fake case laws. Rely strictly on established Indian property law principles."""
        ...
        def invoke_with_retry():
            return chain.invoke({"case_details": str(case_details), "query": query})

        try:
            response = invoke_with_retry()

            return {"success": True, "opinion": response.content, "agent": "Vyasa"}
